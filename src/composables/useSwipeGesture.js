...
export function useSwipeGesture(onCardTap) {
  ...
  function closeSwipe(serviceId) {
    ...
    if (buttons) {
      if (buttons.edit)
        buttons.edit.style.transition =
          "opacity 0.2s ease-out, transform 0.2s ease-out";
      if (buttons.delete)
        buttons.delete.style.transition =
          "opacity 0.2s ease-out, transform 0.2s ease-out";
  ...
  function updateButtonVisibility(serviceId, revealPercent) {
    ...
    if (buttons.edit) {
      buttons.edit.style.willChange =
        revealPercent > 0 ? "opacity, transform" : "auto";
      buttons.edit.style.opacity = opacity;
      buttons.edit.style.transform = `scale(${scale})`;
    }
    if (buttons.delete) {
      buttons.delete.style.willChange =
        revealPercent > 0 ? "opacity, transform" : "auto";
